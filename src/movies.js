...
var movies = [
  ...
  {
    title: 'Pirates f te Caribbean: The Curse of the Black Pearl',
    year: '2003',
...
function turnHoursToMinutes(array) {
  ...
  let result = array.map(function(movie){
    ...
    if (newMovie.duration.length < 3) {
      
      newMovie.duration = parseFloat(newMovie.duration) * 60;
      return newMovie;

    } else if (newMovie.duration.length > 3 && newMovie.duration.length < 8) {
      
      newMovie.duration = parseFloat(newMovie.duration);
      return newMovie;

    } else {
      
      let time = movie.duration.split(" ");
      let hours = parseFloat(time[0]) * 60;
...
function ratesAverage(array) {
  ...
  array.reduce(function(accumulator, currentValue) {
    result = result + currentValue.rate;
  }, 0);
  console.log(parseFloat(result / array.length));
  return parseFloat(result / array.length);
}
...
function dramaMoviesRate(array) {
  ...
  dramaMovies.reduce(function(accumulator, currentValue) {
    result = result + currentValue.rate;
  }, 0);
...
function howManyMovies(array) {
  ...
  let howMany = array.filter(function(element) {
    return (
      element.director === "Steven Spielberg" &&
      element.genre.indexOf("Drama") !== -1
    );
  });

  if (howMany.length === 0) {
    result = "Steven Spielberg directed 0 drama movies!";
  } else if (howMany.length === 1) {
    result = "Steven Spielberg directed 1 drama movies!";
  } else if (howMany.length === 2) {
    result = "Steven Spielberg directed 2 drama movies!";
  } else {
    result = `Steven Spielberg directed ${howMany.length} drama movies!`;
  }
  return result;
}
...
function orderAlphabetically(array) {
  ...
  result = array.filter(function(element, index){
    while(index < 20) {
      return element.title;
    }
    return;
